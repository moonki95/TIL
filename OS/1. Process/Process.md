...
* **프로세스 종류**
  ...
  * **협력 프로세스**
    ...
    * 데이터와 정보를 교환할 수 있는 **프로세스 간 통신** 기법을 필요로 함
    * **생산자 - 소비자 프로세스** 
  * 일반적으로 OS는 한 프로세스가 다른 프로세스의 메모리를 접근하는 것을 금지
* **프로세스 간 통신 종류**
  
  * **공유 메모리 모델**
    * 협력 프로세스들에 의해 공유되는 메모리의 영역이 구축됨
    * 프로세스들은 그 영역에 데이터를 읽고 씀으로써 정보교환 가능
    * 공유 메모리 영역을 구축할 때만 시스템 호출이 필요
    * 데이터의 형식과 위치는 이들 프로세스들에 의해 결정되며 OS의 소관이 아님
  
  * **메시지 전달 모델** 
    * 동일한 주소 공간을 공유하지 않고도 프로세스들이 통신을 하고, 그들의 동작을 동기화할 수 있도록 허용(커널을 통해 메시지 전달)
    * 보통 통신하는 프로세스들이 네트워크에 의해 연결된 다른 컴퓨터들에 존재할 수 있는 분산 환경에서 유용
    * 통신이 협력 프로세스들 사이에 교환되는 메시지를 통하여 이루어짐
    * 충돌을 회피할 필요가 없기 때문에 적은 양의 데이터를 교환하는 데 유용
    * 분산 시스템에서 공유 메모리보다 구현력이 쉬움

  * 메시지 전달이 공유 메모리보다 더 나은 성능을 보임 
    * 공유 메모리는 공유 데이터가 여러 캐시 사이에서 이주하기 때문에 발생하는 캐시 일관성 문제로 인하여 성능 저하 발생
    * 시스템의 처리 코어의 수가 증가하면 할수록 IPC(Inter-Process Communication)로 메시지 전달이 더 선호
    * 
  ![enter image description here](https://s3.us-west-2.amazonaws.com/secure.notion-static.com/64637cf6-b3dc-4e4e-b8c9-7e3e9a0fae5b/Untitled.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20210526%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20210526T054628Z&X-Amz-Expires=86400&X-Amz-Signature=d10acda1c50cae3d3139f06a858e360aa599268bb4b4ae089a7b381a285caab1&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22Untitled.png%22)
  ![enter image description here](https://s3.us-west-2.amazonaws.com/secure.notion-static.com/36c4a3d5-f286-497a-a36d-00c4fc46809c/Untitled.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20210526%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20210526T054803Z&X-Amz-Expires=86400&X-Amz-Signature=c34b2bc2445a3c4a56526187e69c4ee7a2a0aa9f7ea0a1e6009c856f039834db&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22Untitled.png%22)

* **프로세스 간 통신 이유**
  * 정보 공유
    * 여러 사용자가 동일한 정보(공유 파일)에 흥미를 가질 수 있으므로, 그러한 정보를 병행적으로 접근할 수 있는 환경을 만들어 줘야함
  * 계산 가속화
    * 특정 태스크(job)를 빠르게 실행하고자 한다면, 우리는 이걸 서브 태스크로 나누어 각각 다른 서브 태스크들과 병렬로 실행되게 해야함. 
  * 모듈성
    * 시스템 기능을 별도의 프로세스들 또는 스레드들로 나누어서 모듈식 형태로 시스템을 구성하기 위해
  * 편의성
    * 사용자들이 한번에 많은 태스크를 가질수도 있기 때문에 병렬로 처리되도록 환경을 제공해야함

## 11. Naming

* 통신을 원하는 프로세스들은 서로를 가리킬 수 있는 방법이 있어야하는데, 간접통신 혹은 직접통신을 사용할 수 있음

* 직접 통신
  * 통신을 원하는 각 프로세스는 통신의 수신자 또는 송신자의 이름을 명시해야 함 
    * send(p,message) - 프로세스 p에게 message를 전송
    * receive(p, message) - 프로세스 p로부터 message 수신
    * 송신자와 수신자 모두가 프로세스를 통신하려면 이름을 제시해야하는 대칭성을 보임
  * 송신자만 수신자 이름을 지명하며, 수신자는 송신자의 이름을 제시할 필요가 없음
    * send(p,message) - message를 프로세스 p에게 전송
    * receive(id,message) - 임의의 프로세스로부터 message 수신. 변수 id는 통신을 발생시킨 프로세스의 이름 
  * 단점은 프로세스 지정으로 인한 **제한된 모듈성**

* 간접 통신
  * 메일박스 or 포트로 송신하고 이것들로 수신됨
  * 메일박스 
    * 추상적으로 프로세스들에 의해 메시지들이 넣어지고 메시지들이 제거되는 객체
    * 고유의 id를 가짐
    * send(A, message) - message를 메일박스 A로 보냄
    * receive(A, message) - message를 메일박스 A로부터 수신

## 12. 클라이언트 서버 환경 통신

* 공유 메모리 및 메시지 전달 기법을 사용하여 프로세스들이 통신하는 방법 외에 클라이언트 서버 시스템의 통신에서도 사용할 수 있는 세 가지 다른 통신 전략이 있음 (소켓, 원격 프로시저 호출, 파이프)
  * **소켓**
    * 소켓이란 통신의 극점을 뜻함
    * 네트워크를 통하여 통신하는 한 쌍의 프로세스는 프로세스 당 하나씩, 한 쌍의 소켓이 필요
    * 각 소켓은 IP 주소 및 포트 번호 두가지를 접합해서 식별
    * 클라이언트 - 서버 구조 
      * 서버는 지정된 포트에 클라이언트 요청 메시지가 오길 기다림
      * 요청이 수신되면 서버는 클라이언트 소켓으로부터의 연결 요청을 수락함으로써 연결이 완성(Telnet, ftp, http etc)
  * **원격 프로시저 호출(RPC)**
    * 네트워크에 연결되어 있는 두 시스템 사이의 통신에 사용
    * 포트
      * 단순히 메시지 패킷의 시작부분에 포함되는 정수
      * 시스템은 일반적으로 네트워크 주소는 하나를 가지지만, 원격 프로세스가 어떤 서비스를 받고자 한다면 그 서비스에 대응되는 적절한 포트 주소로 메시지를 보내야함 -> 이럴때 포트를 사용해서 정보를 얻을 수 있음
    * 클라이언트가 원격 호스트의 프로시저를 호출하는 것을 마치 자기의 프로시저를 호출하는 것처럼 해줌
  * **파이프**
    * 두 프로세스가 통신할 수 있게 하는 전달자 역할 수행
    * 초기 유닉스 시스템에서 제공하는 IPC 기법중 하나
    * 파이프의 종류
      * 일반 파이프
        * 생산자-소비자 형태로 두 프로세스 간의 통신을 허용
        * 생산자는 파이프의 한 종단에 쓰고, 소비자는 다른 종단에서 읽음
        * **결과적으로 일반 파이프는 한쪽으로만 데이터를 전송할 수 있으며 단방향 통신만 가능**
        * 양방향 통신을 하려면 다른 방향으로 데이터를 전송할 수 있는 두 개의 파이프를 사용해야 함
      * 지명 파이프
        * 일반 파이프는 프로세스들이 통신하는 동안에만 존재하고, 통신이 끝나면 파이프가 사라지게 되는데, 지명 파이프는 양방향으로 가능하며, 생산자-소비자 관계도 필요로 하지 않음
        * 다수의 writer를 가짐
        * 통신 프로세스가 종료되더라도 지명 파이프는 계속 존재